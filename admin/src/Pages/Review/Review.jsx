...
const SkeletonReview = () => (
    <div className="flex gap-4 px-6 py-5 border-b border-gray-100 animate-pulse">
...
/* ── Inline action button (icon-only, tooltip label) ── */
const ActionBtn = ({ icon, label, color, onClick }) => {
    const COLORS = {
        gray: 'text-gray-500 border-gray-200 hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300',
        indigo: 'text-indigo-600 border-indigo-200 hover:text-indigo-700 hover:bg-indigo-50 hover:border-indigo-300',
        emerald: 'text-emerald-600 border-emerald-200 hover:text-emerald-700 hover:bg-emerald-50 hover:border-emerald-300',
        red: 'text-red-500 border-red-200 hover:text-red-700 hover:bg-red-50 hover:border-red-300',
    };
    return (
        <button onClick={onClick} title={label} aria-label={label}
            className={`w-8 h-8 flex items-center justify-center rounded-lg border bg-white shadow-sm transition-all flex-shrink-0 ${COLORS[color] || COLORS.gray}`}>
            {icon}
        </button>
    );
};

/* ── Actions cluster: reused for the desktop column AND the mobile inline row ── */
const ReviewActions = ({ rv, onReply, onApprove, onFlag, onHide, onDelete, className }) => (
    <div className={className}>
        ...
        />
        {rv.status === 'pending' && (
            <ActionBtn icon={<TbCircleCheck size={14} />} label="Approve" color="emerald" onClick={onApprove} />
        )}
        <ActionBtn
            icon={<TbFlag size={14} />}
        ...
        />
        <ActionBtn
            icon={rv.flagged ? <TbEye size={14} /> : <TbEyeOff size={14} />}
            label="Hide"
            color="gray"
            onClick={onHide}
        />
        <ActionBtn icon={<TbTrash size={14} />} label="Delete" color="red" onClick={onDelete} />
    </div>
);
...
const Reviews = ({ token }) => {
    ...
    ════════════════════════════════════════════ */
    return (
        <div className="min-h-screen bg-[#f3f3f1] p-5 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={fetchData} title="Refresh"
                        className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-500 hover:text-indigo-600 hover:border-indigo-300 transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:col-span-2 xl:col-span-2">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="relative flex-1 min-w-[220px] max-w-sm">
                        ...
                        <input
                            ...
                            className="pl-9 pr-8 py-2 rounded-xl border border-gray-300 bg-gray-50 text-[13px] text-gray-800 placeholder-gray-400 outline-none focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-50 transition-all w-full"
                        />
                        {search && (
                    ...
                    <select value={filterRating} onChange={e => applyFilter(setFilterRating, e.target.value)}
                        className="px-3 py-2 pr-8 rounded-xl border border-gray-300 bg-gray-50 text-[13px] text-gray-700 outline-none focus:border-indigo-400 cursor-pointer appearance-none"
                        style={SELECT_STYLE}>
                    ...
                    {productOptions.length > 0 && (
                        <select value={filterProduct} onChange={e => applyFilter(setFilterProduct, e.target.value)}
                            className="px-3 py-2 pr-8 rounded-xl border border-gray-300 bg-gray-50 text-[13px] text-gray-700 outline-none focus:border-indigo-400 cursor-pointer appearance-none max-w-[180px]"
                            style={SELECT_STYLE}>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                {!loading && filtered.length > 0 && (
                    <div className="hidden md:flex items-center gap-4 px-6 py-3 bg-gray-50 border-b border-gray-200 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                        <span className="w-10 flex-shrink-0" />
                        <span className="flex-1">Reviewer & Review</span>
                        <span className="w-40 flex-shrink-0 text-right">Product</span>
                        <span className="w-36 flex-shrink-0 text-right">Actions</span>
                    </div>
                )}
                ...
                ${rv.flagged
                            ...
                            <div className="flex items-start gap-4 px-6 py-5">
                                ...
                                {/* Col 2: Reviewer & Review — flexible, never pushed by product/actions */}
                                <div className="flex-1 min-w-0">
                                    ...
                                    {/* Row 2: stars + rating + date + email (+ product, mobile only) */}
                                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                        ...
                                        <span className="md:hidden text-[11px] text-gray-400 truncate">
                                            on <span className="font-semibold text-gray-600">{rv.productName}</span>
                                        </span>
                                    </div>
                                    ...
                                    {rv.adminReply && !isReplying && (
                                        <div className="mt-3 flex items-start gap-2.5">
                                            ...
                                            <div className="flex-1 bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-2.5">
                                    ...
                                    {/* Action strip — MOBILE ONLY, wraps under content, never full desktop width */}
                                    <ReviewActions
                                        rv={rv}
                                        onReply={handleReplyClick}
                                        onApprove={() => approveReview(rv.id)}
                                        onFlag={() => flagReview(rv.id)}
                                        onHide={() => toggleHide(rv.id)}
                                        onDelete={() => deleteReview(rv.id)}
                                        className="flex md:hidden flex-wrap items-center gap-1.5 mt-3"
                                    />
                                </div>

                                {/* Col 3: Product — fixed width, desktop only */}
                                <div className="hidden md:flex w-40 flex-shrink-0 flex-col items-end text-right gap-1.5">
                                    {rv.productImg
                                        ? <img src={rv.productImg} alt="" className="w-10 h-10 rounded-lg object-cover border border-gray-200" />
                                        : <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center"><TbPhoto size={16} className="text-gray-400" /></div>
                                    }
                                    <div>
                                        <p className="text-[12px] font-semibold text-gray-700 truncate max-w-[150px] leading-tight">{rv.productName}</p>
                                        {rv.productCat && <p className="text-[10.5px] text-gray-400 mt-0.5">{rv.productCat}</p>}
                                    </div>
                                </div>

                                {/* Col 4: Actions — fixed width column, desktop only, never spills full-width */}
                                <ReviewActions
                                    rv={rv}
                                    onReply={handleReplyClick}
                                    onApprove={() => approveReview(rv.id)}
                                    ...
                                    onHide={() => toggleHide(rv.id)}
                                    onDelete={() => deleteReview(rv.id)}
                                    className="hidden md:flex flex-wrap content-start justify-end gap-1.5 w-36 flex-shrink-0"
                                />
                            </div>
                        </div>
                    );
                ...
                {!loading && totalPages > 1 && (
                    <div className="flex items-center justify-between px-6 py-3.5 border-t border-gray-200 bg-gray-50/60">
