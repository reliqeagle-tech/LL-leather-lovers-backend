...
//     const [pricingMode, setPricingMode] = useState("custom"); // "base" or "custom"
...
//             fd.append("price", price); fd.append("discountPrice", discountPrice || ""); fd.append("category", category); fd.append("subCategory", subCategory);
//             fd.append("bestseller", bestseller); fd.append("sizes", JSON.stringify(formatSizes())); fd.append("color", JSON.stringify(colors));
//             fd.append("itemDetails", JSON.stringify(itemDetails)); fd.append("sku", sku);
//             images.forEach(img => { if (img) fd.append("images", img); });
//             const res = await axios.post(`${backendUrl}/api/product/add`, fd, { headers: { token } });
//             if (res.data.success) { toast.success("🎉 Product published!"); try { localStorage.removeItem('ap_draft'); localStorage.removeItem('ap_full_draft'); } catch { } resetForm(); }
//             else toast.error(res.data.message || "Failed to publish");
...
//     const resetForm = () => {
//         setSku(""); setItemDetails([{ title: "", value: "" }]); setPricingMode("custom");
//         setName(""); setDescription(""); setDD(""); setPrice(""); setDiscPrice(""); setColors([]); setEnabledSizes([]); setStdSizes(INIT_SIZES);
...
//                             <Field label="Product Name" required>
//                                 <div className="relative">
//                                     <input className={inputCls + (name.length > 250 ? ' border-red-400 bg-red-50' : '')} type="text" maxLength={250} placeholder="e.g. Classic Oxford Cotton Shirt" value={name} onChange={e => setName(e.target.value)} />
//                                     <span className={`absolute right-3 bottom-3 text-[10.5px] pointer-events-none ${name.length > 250 ? 'text-amber-500' : 'text-gray-400'}`}>{name.length}/250</span>
//                                 </div>
//                             </Field>

//                             <Field label="Short Description" required>
//                                 <div className="relative">
//                                     <textarea className={inputCls + " resize-y min-h-[90px] " + (description.length > 300 ? 'border-red-400 bg-red-50' : '')} maxLength={300} placeholder="Compelling product description…" value={description} onChange={e => setDescription(e.target.value)} />
//                                     <span className={`absolute right-3 bottom-3 text-[10.5px] pointer-events-none ${description.length > 300 ? 'text-amber-500' : 'text-gray-400'}`}>{description.length}/300</span>
//                                 </div>
//                             </Field>
...
//                                             if (!CATEGORY_DEFAULT_SUB[newCat]) {
//                                                 setSubCategory('');
//                                             }
//                                         }}
//                                     >
...
//                                 <Field label="SKU / Code" hint="Mandatory for inventory management">
//                                     <input value={sku} onChange={e => setSku(e.target.value)} className={inputCls} type="text" placeholder="WHITE-BEAR-(SQ)-PK2" />
//                                 </Field>
//                             </div>
...
//             className="w-full py-3 rounded-xl border-2 border-dashed border-gray-200 text-[13px] font-semibold text-gray-500 hover:border-indigo-300 hover:text-indigo-500 transition-all"
//         >
//             + Add Item Detail
//         </button>
//     </div>
