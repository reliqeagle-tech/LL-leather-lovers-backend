  ...
  return (
    <>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        {/* Product – now clickable */}
        <TableCell
          sx={{ cursor: "pointer" }}
          onClick={() => navigate(`/product/${item.productId}`)}   // ← navigation added
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg overflow-hidden bg-white/5 border border-white/10 shrink-0">
              <img src={item.image} alt={item.name}
            ...
            <Tooltip title="View product details" arrow placement="top">
              <span
                className="line-clamp-2 text-indigo-400 hover:text-indigo-300 transition-colors"
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                {/* Address, Payment, Pricing – unchanged */}
        ...
        response.data.orders.forEach((order) => {
          order.items.forEach((item) => {
            flatList.push({
              ...
              orderId: order._id,
              userId: order.userId,
              productId: item._id,               // ← must be product _id
              orderTotal: Number(order.finalAmount),
              paymentMethod: order.paymentMethod,
              payment: order.payment,
              paymentId: order.paymentId,
  ...
  return (
    <ThemeProvider theme={darkTheme}>
      <div style={{ background: "linear-gradient(180deg, #08080f 0%, #0b0b14 100%)" }}
        ...
        {/* ... rest of your layout (heading, sidebar, table, empty state) remains same ... */}
        ...
        {/* ... empty state, refresh button, etc. remain unchanged ... */}
      </div>
    </ThemeProvider>
  );
};
